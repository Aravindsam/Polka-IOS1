...
+(void)show { [[self sharedView] setFrame: [[UIScreen mainScreen] bounds]]; [[self sharedView] showProgress:-1 status:nil maskType:SVProgressHUDMaskTypeNone]; }

//+ (void)showWithStatus:(NSString *)status {
//    [[self sharedView] showProgress:-1 status:status maskType:SVProgressHUDMaskTypeNone];
//}

+(void)showWithStatus:(NSString *)status { [[self sharedView] setFrame: [[UIScreen mainScreen] bounds]]; [[self sharedView] showProgress:-1 status:status maskType:SVProgressHUDMaskTypeNone]; }
...
+(void)showWithMaskType:(SVProgressHUDMaskType)maskType
{
    [[self sharedView] setFrame: [[UIScreen mainScreen] bounds]];
    [[self sharedView] showProgress:-1 status:nil maskType:maskType];
}
    ...
    if(string) {
        ...
        if ([string respondsToSelector:@selector(boundingRectWithSize:options:attributes:context:)]) {
          stringRect = [string boundingRectWithSize:constraintSize
            ...
            float version_os=[[[UIDevice currentDevice] systemVersion] floatValue];
//            #ifdef __IPHONE_8_0
            if(version_os >= 8.0)
